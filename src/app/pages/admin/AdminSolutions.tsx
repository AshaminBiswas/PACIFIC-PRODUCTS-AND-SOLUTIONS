        ...
        <div className="fixed inset-0 z-[100] flex items-start justify-center bg-black/70 overflow-y-auto p-4 pt-12">
          <div className="w-full max-w-2xl bg-[#0a0a1a] border border-white/10 rounded-2xl">
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div><label className="block text-sm text-gray-300 mb-1">Title *</label><select value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} className="w-full px-4 py-2.5 bg-[#0a0a1a] border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-[#7FB706]"><option value="">Select an Industry</option><option value="Corporates">Corporates</option><option value="Malls">Malls</option><option value="Airports">Airports</option><option value="Metro and railways">Metro and railways</option><option value="Hospitals">Hospitals</option><option value="Schools & Colleges">Schools & Colleges</option><option value="Others">Others</option></select>
              {form.title === "Others" && <input type="text" value={customTitle} onChange={e => setCustomTitle(e.target.value)} placeholder="Enter custom industry" className="w-full mt-2 px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-[#7FB706]" />}</div>
